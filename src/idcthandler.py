    ...
    def getResult(self, grid, idct_choice='interval', poly2cheb_choice='interval'):
        ...
        if (grid.lower_x != grid.upper_x):
            ...
            with Timer("evaluation", logger=None):
                r_z = cfg.functions['idct'][0](p_z, n=grid.zero['N_x'])[grid.zero['i_min_x']:grid.zero['i_max_x']+1]
        ...
        if (grid.x_min != grid.lower_x):
            ...
            with Timer("evaluation", logger=None):
                p_m = cfg.functions['idct'][0](p_inv, n=grid.minus['N_x'])[grid.minus['i_min_x']:grid.minus['i_max_x']+1]
            with Timer("change", logger=None):
        ...
        if (grid.upper_x != grid.x_max):
            ...
            with Timer("evaluation", logger=None):
                p_p = cfg.functions['idct'][0](p_inv, n=grid.plus['N_x'])[grid.plus['i_min_x']:grid.plus['i_max_x']+1]
            with Timer("change", logger=None):

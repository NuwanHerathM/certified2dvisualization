...
p = np.empty((n,deg_x+1))
for j in range(deg_x+1):
    _p = np.polynomial.chebyshev.poly2cheb(poly[j])
    p[:,j] = corrected_idct(_p, n)

# for j in range(deg_x+1):
#     _p = np.polynomial.Polynomial(poly[j])
#     print(_p)
#     p[:,j] = [_p(e) for e in grid]
...
for i in range(n):
    ...
    for j in range(n):
        if vanishes(ft.arb_poly(p_der[:,j].tolist()), a, ft.arb(grid[j]), grid, j, m, d):
            l.append((i,j))
